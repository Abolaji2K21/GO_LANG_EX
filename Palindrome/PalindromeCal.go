...
func main() {
	var userInput int
	fmt.Print("Enter a five digits number to check if integer is a palindrome: ")
	...
	for userInput != -1 {
		if userInput < 10000 || userInput > 99999 {
			...
			if userInput == userInput6 {
				fmt.Printf("Hello! Your inputted integer %d is a palindrome.\n", userInput)
				fmt.Println()
			} else {
				fmt.Printf("Hello! The five digit you entered %d is not a palindrome number.\n", userInput)
				fmt.Println()
			}
		}
		fmt.Print("Enter a five digits number to check if integer is a palindrome: ")
	...
	if userInput == -1 {
		fmt.Println("Thanks for checking. I am always here to help with your curiosity.")
	}
}
